...
function testMainIPCHandlers() {
  ...
  const details = passed 
    ? 'IPC handler configured' 
    : `Handler: ${hasHandler}, Shell: ${hasShell}`;
  return [passed, details];
}
...
function testStylesExist() {
  ...
  const missing = styles.filter(file => 
    !fs.existsSync(path.join(__dirname, file))
  );
...
async function main() {
  ...
  if (exitCode === 0) {
    console.log(`${colors.green}${colors.bright}✓ All tests passed! Desktop app is ready.${colors.reset}\n`);
    ...
    console.log(`${colors.red}${colors.bright}✗ Some tests failed. Fix the issues above and try again.${colors.reset}\n`);
  }
